...
func init() {
	...
	upCmd.PersistentFlags().StringVarP(&server.Static, "dir", "i", "pwd", "Default url for current working dir, e.g. /pwd")
	...
	upCmd.PersistentFlags().StringSliceVarP(&server.Exclude, "exclude", "e", []string{".git"}, "Exclude those dirs, e.g. \".git,.idea\"")
